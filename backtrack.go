...
func (b *bitState) pushPossessive(re *Regexp, pc uint32, pos int, arg bool) {
	// Only check shouldVisit when arg is false.
	// When arg is true, we are continuing a previous visit.
	if re.prog.Inst[pc].Op != syntax.InstFail && (arg || b.shouldVisit(pc, pos)) {
		if len(b.jobs) > 0 && b.jobs[len(b.jobs)-1].posId == re.prog.Inst[pc].PosId {
			b.jobs = b.jobs[0 : len(b.jobs)-1]
		}
		b.jobs = append(b.jobs, job{pc: pc, arg: arg, pos: pos, posId: re.prog.Inst[pc].PosId})
	}
}
	...
	Skip:
		...
		case syntax.InstAltPos:
			if arg {
				...
				b.pushPossessive(re, pc, pos, true)
				pc = inst.Out
				goto CheckAndLoop
			}
