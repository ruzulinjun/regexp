...
func (c *compiler) compile(re *Regexp) frag {
	...
	case OpStarPos:
		return c.starPos(c.compile(re.Sub[0]), re.Flags&NonGreedy != 0)
	case OpPlusPos:
		return c.plusPos(c.compile(re.Sub[0]), re.Flags&NonGreedy != 0)
	case OpQuestPos:
		return c.questPos(c.compile(re.Sub[0]), re.Flags&NonGreedy != 0)
	case OpConcat:
		if len(re.Sub) == 0 {
			return c.nop()
		}
...
func (c *compiler) starPos(f1 frag, nongreedy bool) frag {
	f := c.inst(InstAltPos)
	i := &c.p.Inst[f.i]
	c.posId++
	i.PosId = c.posId
...
func (c *compiler) questPos(f1 frag, nongreedy bool) frag {
	f := c.inst(InstAltPos)
	i := &c.p.Inst[f.i]
	c.posId++
	i.PosId = c.posId
...
func (c *compiler) plusPos(f1 frag, nongreedy bool) frag {
	return frag{f1.i, c.starPos(f1, nongreedy).out}
}
